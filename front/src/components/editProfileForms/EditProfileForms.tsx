...
import { EditProfileFormsProps, EditInfosFormValues, EditPasswordFormValues } from './types';
...
export const EditInfos = (props: EditProfileFormsProps) => {
	...
	const submit = async (model?: EditInfosFormValues) => {
		try {
			...
			if (res.status === 200) {
				...
				setAlertsContext({
					type: 'success',
					message: 'Le changement de vos informations a bien été pris en compte !',
				});
	...
	return (
		<>
			<div className={`${classes.formContainer} ${openConfirm && classes.hidden}`}>
				...
				<form className={classes.form}>
					<TextFieldController
						...
						control={control}
						rules={{ required: 'Veuillez indiquer votre adresse mail' }}
					/>
					<TextFieldController
						name='firstName'
						label='Prénom'
...
export const EditPassword = (props: EditProfileFormsProps) => {
	...
	React.useEffect(() => {
		if (Object.keys(watchAllFields).length) {
			...
			if (!password || !newPassword || !confirmPassword || errorsNumber > 0) {
				setCanSubmit(false);
			} else {
				setCanSubmit(true);
			}
	...
	const checkErrorsAndCredentials = async (model: EditPasswordFormValues) => {
		...
		if (password === newPassword) {
			...
			try {
				const credentials = { email: userContext?.email || '', password };
	...
	const submit = async (newPassword: string) => {
		try {
			const res = await axios.patch('/users/password', { password: newPassword });
			if (res.status === 200) {
				...
				setAlertsContext({
					type: 'success',
					message: 'Le changement de votre mot de passe a bien été pris en compte !',
				});
			}
		} catch (error) {
			setAlertsContext({
		...
		<div className={classes.formContainer}>
			...
			<form className={classes.form}>
				<TextFieldController
					...
					defaultValue=''
					control={control}
					rules={{ required: 'Veuillez entrer le mot de passe actuel' }}
					type='password'
				/>
				<TextFieldController
					name='newPassword'
					...
					defaultValue=''
					control={control}
					rules={{ required: 'Veuillez entrer le nouveau mot de passe' }}
					type='password'
				/>
				<TextFieldController
					...
					defaultValue=''
					control={control}
					rules={{ required: 'Veuillez confirmer le nouveau mot de passe' }}
					type='password'
				/>
			</form>
			<div className={classes.buttonsContainer}>
				<Button onClick={() => setSelectedForm(null)} variant='outlined' className={classes.button}>
					Annuler
				</Button>
				<Button
		...
		<>
			<div className={`${classes.formContainer} ${openConfirm && classes.hidden}`}>
				...
				<div className={classes.infoText}>
					<span>
						<strong>Attention</strong> : la suppression du compte est définitive.
					</span>
				...
				<div className={classes.buttonsContainer}>
					...
					<Button onClick={toggleConfirmDialog} variant='contained' className={classes.button}>
						Oui
					</Button>
				</div>
			</div>
