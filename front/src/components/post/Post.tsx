...
const Post = (props: PostProps) => {
	...
	const handleDelete = async () => {
		try {
			...
			if (res.status === 200) {
				setAlertsContext({ type: 'success', message: 'La publication a bien été supprimée !' });
				handleClose();
				updateFeed();
			}
		} catch (error) {
			setAlertsContext({
				type: 'error',
				message: "Un problème est survenu, la publication n'a pas pu être supprimée",
			});
		}
	};
	...
	const submitPostUpdate = async () => {
		try {
			const res = await axios.patch(`/posts/${id}`, { text: updatedText });
			if (res.status === 200) {
				setAlertsContext({ type: 'success', message: 'La publication a bien été modifiée !' });
				hideUpdate();
				updateFeed();
			}
		} catch (error) {
			setAlertsContext({
				type: 'error',
				message: "Un problème est survenu, la modification n'a pas pu être enregistrée",
			});
		}
	};
	...
	const submitComment = async () => {
		try {
			...
			if (res.status === 201) {
				setAlertsContext({ type: 'success', message: 'Le commentaire a bien été enregistré !' });
				setCommentText('');
				updateFeed();
			}
		} catch (error) {
			setAlertsContext({
				type: 'error',
				message: "Un problème est survenu, le commentaire n'a pas pu être enregistré",
			});
		}
	};
		...
		<div className={classes.postContainer}>
			<Paper elevation={0} className={classes.post}>
				<div className={classes.header}>
					...
					{userContext && (userContext.id === author.id || userContext.isAdmin) && (
						<div>
							<Button onClick={e => handleClick(e)} className={classes.menuBtn}>
								<MoreHorizIcon />
							</Button>
							<Menu id='basic-menu' anchorEl={anchorEl} open={menuOpen} onClose={handleClose}>
								{userContext.id === author.id && <MenuItem onClick={openUpdate}>Modifier</MenuItem>}
				...
				<div className={classes.reactions}>
					<span className={classes.likesCount}>
						<ThumbUpAltIcon fontSize='small' sx={{ marginRight: '4px' }} />
						{likes?.length}
					</span>
					<span
				...
				<div className={classes.reactOptions}>
					<Button
						className={[classes.likes, hasUserLikedPost && classes.liked].join(' ')}
						onClick={handleLike}
					>
						{hasUserLikedPost ? (
							<ThumbUpAltIcon fontSize='small' sx={{ marginRight: '4px' }} />
						) : (
							<ThumbUpOffAltIcon fontSize='small' sx={{ marginRight: '4px' }} />
						)}
						J'aime
					</Button>
					<Button className={classes.likes} onClick={() => toggleComments(true)}>
						<ChatBubbleOutlineIcon fontSize='small' sx={{ marginRight: '4px' }} />
						Commenter
					</Button>
				</div>
				{showComments && (
					...
					<>
						...
						<div className={classes.commentsContainer}>
							{comments.map(comment => (
								<Comment key={comment.id} comment={comment} updateFeed={updateFeed} />
							))}
						</div>
					</>
				)}
