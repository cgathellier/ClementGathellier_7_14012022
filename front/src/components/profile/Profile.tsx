...
import { EditInfos, EditPassword, DeleteAccount } from '../editProfileForms/EditProfileForms';
	...
	const getUserInfos = async () => {
		try {
			...
			if (res.status === 200) {
				...
				setAlertsContext({
					type: 'error',
					message: 'Impossible de charger les informations du profil, merci de réessayer plus tard',
				});
			}
		} catch (error) {}
	};
	...
	React.useEffect(() => {
		switch (selectedForm) {
			case 'infos':
				...
				break;
			case 'password':
				setFormShown(<EditPassword setSelectedForm={setSelectedForm} />);
				break;
			case 'delete':
				setFormShown(<DeleteAccount setSelectedForm={setSelectedForm} />);
				break;
			default:
				break;
		}
	...
	return (
		<div className={classes.profile}>
				...
				<div className={classes.userInfosContainer}>
					...
					{userContext?.id === userInfos.id && (
						<div className={classes.openEditMenuButtonContainer}>
							<Button onClick={toggleEditMenu} variant='contained' startIcon={<EditIcon />}>
								Gérer le compte
							</Button>
						</div>
					)}
			...
			<div className={classes.feedContainer}>
				<Feed posts={posts} profileId={userInfos?.id} updateProfile={getUserInfos} />
			</div>
			{userContext?.id === userInfos?.id && (
				<>
					<div
					...
					></div>
					<div className={`${classes.editMenuContainer} ${showEditMenu && classes.editMenuShown}`}>
						{showEditMenu && (
							<FocusTrap
								focusTrapOptions={{
							...
							>
								<div className={classes.editMenuContentContainer}>
									{selectedForm ? (
										<div>{formShow}</div>
									) : (
										<div className={classes.editMenuButtonsContainer}>
											<Button
												onClick={() => setSelectedForm('infos')}
												variant='contained'
												fullWidth
												action={actionRef}
												onFocus={handleFocus}
									...
									<Button
										...
										sx={{ color: '#091f43' }}
										className={classes.closeEditMenuIconContainer}
									>
