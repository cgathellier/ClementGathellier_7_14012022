...
const Comment = (props: CommentProps) => {
	...
	const handleDelete = async () => {
		try {
			...
			setAlertsContext({
				type: 'error',
				message: "Un problème est survenu, le commentaire n'a pas pu être supprimé",
			});
		}
	};
	...
	const submitCommentUpdate = async () => {
		try {
			const res = await axios.patch(`/comments/${id}`, { text: updatedText });
			if (res.status === 200) {
				setAlertsContext({
					type: 'success',
		...
		<div className={classes.commentContainer}>
			<div className={classes.content}>
				<div className={classes.header}>
					...
					>
						{author ? `${author.firstName} ${author.lastName}` : 'Utilisateur supprimé'}
					</span>
					{userContext && (userContext.isAdmin || (author && userContext.id === author.id)) && (
						<div>
							...
							<Menu id='basic-menu' anchorEl={anchorEl} open={menuOpen} onClose={handleClose}>
								{author && userContext.id === author.id && (
									<MenuItem onClick={openUpdate}>Modifier</MenuItem>
								)}
								{((author && userContext.id === author.id) || userContext.isAdmin) && (
				...
				{likes.length > 0 && (
					<span className={classes.likes}>
						...
						<div className={classes.likesCount}>{likes?.length}</div>
					</span>
				)}
			</div>
			<div
